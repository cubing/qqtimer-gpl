...
SlidySolver.prototype.resetTilesToRegion = function (region) {
  for (var i = 0; i < this.n; i++) {
    if (
      this.contains(this.regions[region], i + 1) ||
      this.contains(this.fixedTiles[region], i + 1)
    )
      this.tiles[i] = i + 1;
    else this.tiles[i] = 0;
  }
...
SlidySolver.prototype.encode = function (r) {
  ...
  for (var i = 0; i < regsize; i++) {
    positions[i] = this.indexof(
      this.movableTiles[r],
      this.indexof(this.tiles, this.regions[r][i]) + 1
    );
  }
  for (var i = 0; i < regsize; i++) {
    positions2[i] = positions[i];
    for (var j = 0; j < i; j++)
      if (positions[j] < positions[i]) positions2[i]--;
  }
...
SlidySolver.prototype.decode = function (t, r) {
  ...
  for (var i = regsize - 1; i >= 0; i--) {
    for (var j = regsize - 1; j > i; j--) {
      if (positions[j] >= positions[i]) positions[j]++;
    }
  }
  for (var i = 0; i < regsize; i++) {
  ...
  for (var i = 0; i < this.n; i++) {
    if (this.contains(this.fixedTiles[r], i + 1)) this.tiles[i] = i + 1;
    else this.tiles[i] = 0;
  }
  for (var i = 0; i < regsize; i++) {
    this.tiles[positions[i]] = this.regions[r][i];
    if (this.tiles[positions[i]] == this.n) this.gap = positions[i];
  }
};
...
SlidySolver.prototype.canDoMove = function (g, m) {
  if (m == 0) return g < this.width * (this.height - 1);
  else if (m == 1) return g % this.width != this.width - 1;
  else if (m == 2) return g >= this.width;
  else if (m == 3) return g % this.width != 0;
  return false;
};
...
SlidySolver.prototype.move = function (t, g, m) {
  if (!this.canDoMove(g, m)) return false;
...
SlidySolver.prototype.maketables = function () {
  ...
  for (var reg = 0; reg < this.regions.length; reg++) {
    ...
    for (var tile = 0; tile < this.n; tile++) {
      ...
      if (
        this.tiles[tile] == 0 &&
        !this.contains(this.fixedTiles[reg], tile + 1)
      ) {
...
SlidySolver.prototype.scramble = function () {
  for (var i = 0; i < this.n; i++) this.tiles[i] = i + 1;
  this.gap = this.n - 1;
  var a,
    d,
    parity = 0;
...
SlidySolver.prototype.getscramble = function () {
  ...
  var currentDepth;
  var scrambledPuzzle = [];
  for (var i = 0; i < this.n; i++) scrambledPuzzle.push(this.tiles[i]);
  ...
  for (var i = 0; i < this.regions.length; i++) {
    ...
    for (var j = 0; j < this.n; j++) {
      if (
        this.contains(this.regions[i], scrambledPuzzle[j]) ||
        this.contains(this.fixedTiles[i], scrambledPuzzle[j])
      )
...
SlidySolver.prototype.init = function () {
  ...
  for (var i = 0; i < this.regions.length; i++) {
    ...
    for (var j = 1; j <= this.n; j++) {
      if (!this.contains(this.fixedTiles[i], j)) this.movableTiles[i].push(j);
    }
  }
  for (var i = 0; i < this.regions.length; i++) this.regions[i].push(this.n);
  for (var i = 0; i < this.n; i++) this.tiles.push(0);
};
